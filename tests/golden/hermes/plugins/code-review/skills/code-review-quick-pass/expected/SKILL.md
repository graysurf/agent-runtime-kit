...
- Run the review through the managed read-only `reviewer-quick` subagent
  whenever the active host exposes subagent dispatch. Use `delegate_task` to dispatch `reviewer-quick` when subagent dispatch is
  available; inline review is the fallback.
...
4. Use `delegate_task` to dispatch a read-only `reviewer-quick` subagent with
   the base ref and any focus notes. The subagent inspects read-only and returns
   a compact verdict with source-grounded findings and residual risks. You stay
   the parent: you own base-ref selection, synthesis of the returned findings,
   the escalation decision, and every provider / merge action.
