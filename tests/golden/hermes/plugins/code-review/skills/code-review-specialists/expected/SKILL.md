...
- Dispatch the managed read-only `reviewer-<lens>` subagents for the selected
  lenses whenever the active host exposes subagent dispatch. Use `delegate_task`
  when it is available to dispatch the selected reviewers; inline lens execution
  is only the fallback when dispatch is unavailable or blocked, and the fallback
  must be stated. The parent agent owns lens selection, dispatch, validation,
  and merge.
...
5. Select the matching managed reviewer subagents for the chosen lenses
   ...
   available through the managed reviewer subagent index.
6. Dispatch the selected read-only reviewer subagents one per lens, handing each
   ...
   its lens. Use `delegate_task` when it is available. If dispatch is unavailable
   or blocked, state the fallback reason and run the same lenses inline by
   reading the prompt from `references/specialists/`. You stay the parent: you
   own base-ref selection, lens selection, dispatch, fallback justification, and
   the validation/merge steps below.
