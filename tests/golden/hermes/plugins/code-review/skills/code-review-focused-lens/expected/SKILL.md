...
- Dispatch the matching managed `reviewer-<lens>` subagents whenever the active
  host exposes subagent dispatch. Use `delegate_task` when it is available to
  dispatch the requested reviewers; inline review is only the fallback when
...
4. Dispatch the matching managed reviewer subagent for each requested lens
   ...
   dispatch is available. Use `delegate_task` when it is available.
