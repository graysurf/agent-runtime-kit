#!/usr/bin/env bash
# Cut and verify the agent-runtime-kit GHCR image release.
...
Options:
  ...
  --version VERSION      CalVer date, YYYY.MM.DD or vYYYY.MM.DD.
...
normalize_version() {
  ...
  if [[ ! "$raw" =~ ^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$ ]]; then
    die "version must match YYYY.MM.DD or vYYYY.MM.DD: $1"
  fi
  printf '%s\n' "$raw"
}
...
verify_public_manifest() {
  ...
  token="$(curl -fsSL \
    ...
    json_token)"
  [ -n "$token" ] || die "could not fetch anonymous GHCR pull token for $repo"

  manifest="$(curl -fsSL \
    -H "Authorization: Bearer $token" \
    -H 'Accept: application/vnd.oci.image.index.v1+json, application/vnd.docker.distribution.manifest.list.v2+json, application/vnd.oci.image.manifest.v1+json, application/vnd.docker.distribution.manifest.v2+json' \
    "https://ghcr.io/v2/${repo}/manifests/${tag}")" ||
    die "anonymous GHCR manifest read failed for ghcr.io/${repo}:${tag}"

  platforms="$(printf '%s' "$manifest" | json_platforms)"
  case " $platforms " in
    *" linux/amd64 "*) ;;
    *) die "ghcr.io/${repo}:${tag} missing linux/amd64 platform (platforms: ${platforms:-none})" ;;
  esac
  case " $platforms " in
    *" linux/arm64 "*) ;;
    *) die "ghcr.io/${repo}:${tag} missing linux/arm64 platform (platforms: ${platforms:-none})" ;;
  esac

  info "verified public GHCR manifest: ghcr.io/${repo}:${tag} platforms=${platforms}"
}
