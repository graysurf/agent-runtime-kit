...
- `v1.9.1` is a lock-step host bump over `v1.8.0`. It restructures one surface
  ...
  - `forge-cli pr review-threads` becomes a subcommand group. The read surface
    ...
    subcommands are added: `pr review-threads resolve <thread-id>` (optionally
    `--note` to reply before resolving, idempotent) and `pr review-threads
    reply <thread-id>` (reply without resolving); both return
