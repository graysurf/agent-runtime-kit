...
- `v1.8.0` is a lock-step host bump over `v1.7.1`. It refreshes two surfaces
  ...
  - `evidence migrate` tightens cwd/origin and slug matching for repo identity:
    ...
    safety. The `evidence-migrate` and `heuristic-session-closeout` retention
    lanes consume those guarantees, so the `evidence` floor moves to `1.8.0`
...
- `v1.0.15` tightens the `agent-runtime pr-body render` input contract
  ...
  silently dropped. Runtime-kit `pr:create-pr` / `pr:deliver-pr` /
  `pr:pr-lifecycle` docs adopted the surface in agent-runtime-kit commit
...
- Prior pin: `v0.29.0` at `0f757df` (`chore(release): bump cli versions to
  ...
  Runtime-kit now consumes `agent-memory index global` opportunistically in the
  Codex UserPromptSubmit memory cue hook; the hook fails open when the binary or
  memory index is unavailable and reminds Codex to propose durable memory
  updates only with explicit user approval. Earlier release history retained
  below.
  `v0.25.8` at `4d0d621`
...
| `claude-cli`                | `claude-cli`                                                                                                        | Claude runtime helper. As of `v1.0.12`, ships native prompt-segment auth/cache/render/status helpers plus generated completions. Runtime-kit does not consume this surface in required skill flows today. |
