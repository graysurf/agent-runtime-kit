...
- `v1.0.2` adds the **`heuristic-inbox deliver`** subcommand: a cwd-independent
  ...
  `heuristic-session-closeout` skill-prose writeback delivered in #237. As of
  `v1.0.3`, `deliver` also takes a repeatable `--label` (forwarded to
  `forge-cli pr create --label`); the closeout skill consumes
  `deliver --label workflow::heuristic-records`, so the `heuristic-inbox >=
  1.0.3` `required_clis[]` floor and the `heuristic-system` surface
  `min_nils_cli` (`v1.0.3`) are set
  ([#745](https://github.com/sympoies/nils-cli/pull/745),
  [#748](https://github.com/sympoies/nils-cli/pull/748)).
