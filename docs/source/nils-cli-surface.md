...
- `v1.0.9` is a **patch** over `zsh-kit setup --write-zshenv`: the managed
  ...
  `>= 1.0.9`.
- After `v1.0.9`, bootstrap hardening work landed the additive
  `agent-runtime bootstrap-host` command and
  `agent-runtime.bootstrap-report.v1` checkpoint/report schema
  ([#780](https://github.com/sympoies/nils-cli/pull/780),
  [#781](https://github.com/sympoies/nils-cli/pull/781)). This repo's
  `scripts/setup.sh` feature-detects that command and delegates runtime
  surface render / install / prune-stale / skill-surface verification when the
  installed nils-cli surface provides it. While the active pin remains
  `v1.0.9`, setup stays compatible through explicit phase-command fallback.
...
| `agent-runtime`         | `agent-runtime`                                                                                                     | Runtime kit CLI. As of `v0.20.0`, this repo consumes released `render`, `install`, `uninstall`, `doctor` (including `--class skill-surface --product codex`), `audit-drift`, `gc-backups`, `restore-backups`, `purge-state`, and `pr-body render` bodies through Homebrew. The `pr-body render` surface renders standardized feature / bug PR and MR bodies before `forge-cli pr create` / `forge-cli pr deliver`. As of `v0.22.4`, `sync-runtime-surfaces` consumes `agent-runtime prune-stale` to remove stale managed Codex and Claude skill surfaces after install. As of `v0.28.0`, ships `doctor --class version-alignment --pin <manifest>` (the surface-pin drift gate this repo's Position 2 consumes via `docs/source/nils-cli-pin.yaml`) and adds build metadata to the `agent-runtime --version` output. As of `v1.0.5`, `render` reconciles `build/<product>/` for retired skills — a skill removed from the manifest has its outputs and `.render-cache.json` entry dropped on the next render, so `sync-runtime-surfaces` + `prune-stale` no longer leave the retired skill in the live home ([#755](https://github.com/sympoies/nils-cli/pull/755)); `audit-drift` also gains `--json` / `--fail-on` and skips path/slug runs in entropy ([#754](https://github.com/sympoies/nils-cli/pull/754)). After `v1.0.9`, `bootstrap-host` adds a single dry-run/apply wrapper over render, install, prune-stale, and skill-surface doctor plus a checkpoint/report schema; runtime-kit setup feature-detects it and keeps a phase-command fallback until the surface pin moves past `v1.0.9` ([#780](https://github.com/sympoies/nils-cli/pull/780), [#781](https://github.com/sympoies/nils-cli/pull/781)). |
