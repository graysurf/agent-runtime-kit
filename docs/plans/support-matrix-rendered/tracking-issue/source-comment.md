...
- Path: `docs/plans/support-matrix-rendered/support-matrix-rendered-discussion-source.md`
...
- Source: in-session discussion immediately following the closeout of
  the previous `docs/plans/support-matrix/` plan (issue
...
- [U3] User asked for this work to be tracked through the same
  ...
  `docs/plans/support-matrix/` plan, including the
...
- [F3] The closed plan's `F4` ("no parsing the markdown view")
  ...
  cited verbatim in `docs/plans/support-matrix/support-matrix-discussion-source.md:75-79`.
...
- Drive `create-plan-tracking-issue` against
  ...
  `Read First`.
- Keep the closed plan (`docs/plans/support-matrix/`) cited as
...
- Retain this discussion source under
  `docs/plans/support-matrix-rendered/` until the plan it feeds
