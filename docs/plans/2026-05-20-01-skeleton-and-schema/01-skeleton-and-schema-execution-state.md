...
> **Live state lives on the tracking issue.** This file captures the
> initial pre-execution snapshot. The authoritative execution state is the
> latest `execute-from-tracking-issue:state:v1` comment on
> [graysurf/agent-runtime-kit#1](https://github.com/graysurf/agent-runtime-kit/issues/1)
> (current status at 2026-05-20: in-flight — Tasks 3.3 + 3.4 execute as a
> coupled `sympoies/nils-cli` `v0.12.0` workspace bump that brings the
> stub `agent-runtime-cli` crate inline with the rest of the workspace).
> Append-only `:session:v1` and
> `:validation:v1` comments on the same issue carry the per-sprint
> evidence; do not edit this file to chase live state.

## Current State (initial snapshot, pre-execution)
...
- Source document: docs/plans/2026-05-20-01-skeleton-and-schema/01-skeleton-and-schema-plan.md
...
| Task 3.3 | pending | Cut `v0.12.0` nils-cli release (workspace bump)               | n/a      | cross-repo: `sympoies/nils-cli`; coupled-workspace bump (every crate `0.11.0`/`0.0.1-dev` → `0.12.0`); record tag, release URL, tarball SHA256 here |
...
| --- | --- | --- | --- |
| `plan-tooling validate --file docs/plans/2026-05-20-01-skeleton-and-schema/01-skeleton-and-schema-plan.md --format text --explain` | pending | run before first commit | n/a |
...
| `brew update && brew reinstall sympoies/tap/nils-cli && agent-runtime --version \| grep -F 0.12.0` | pending | Sprint 3 — install ladder reachable | n/a |
