...
| --- | --- | --- | --- | --- | --- |
| high | F1 | `agent-runtime install` body is stubbed (Plan 01 Sprint 3 cut a `0.0.1-dev` no-op). Without it Phase 3 cannot ship the reporting POC into a live home. | Phase 3 bullet 1; `## Install And Link Strategy` | `nils-cli/crates/agent-runtime-cli/src/install.rs` | Dry-run prints a deterministic plan; `--apply` links approved files, syncs managed blocks, and backs up replaced originals; `--live-home` redirects to a sandbox path. |
