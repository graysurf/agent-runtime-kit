...
| 15. Codex local skill root (`$CODEX_HOME/skills/<d>/<s>/`) | codex | shipped | one directory symlink per active skill folder under `$CODEX_HOME/skills/<d>/<s>/` | `core/skills/<domain>/<skill>/` → `build/codex/plugins/<d>/skills/<s>/` (44 entries) | 0.130.0 | v0.17.5 | gate 2 (render), gate 4 (golden), gate 5 (drift), gate 6 (`doctor --class skill-surface --product codex`), gate 7 (sandbox), gate 8 (runtime-smoke) | `codex debug prompt-input` fresh session | `targets/codex/link-map.yaml:7-12, 28-44, 57-127, 343-389`; `manifests/skills.yaml:12-650` |
...
- **Codex-side skill-surface doctor exists; Claude side does not.**
  ...
  the default CI gate stack (position 6). There is no
...
- Manifest counts referenced inline change — e.g. `manifests/plugins.yaml`
  grows past 10 plugin entries, or `manifests/skills.yaml` past 44
